...
import com.mfq.dataservice.cache.impl.transcoders.CachedData;
 ...
 * @sine 2015-08-05
 */
public class RedisCacheManipulater extends BaseCache implements IRedis, RedisCacheMBean {

    private ShardedJedisPool pool;

    private SerializingTranscoder transcoder = new SerializingTranscoder();

    private AtomicInteger maxPoolCount = new AtomicInteger(0);

    private AtomicInteger currentPoolCount = new AtomicInteger(0);

    private AtomicLong totalCallTimes = new AtomicLong();

    /**
     * 为了支持线程安全，被迫使用线程栈
     */
    ThreadLocal<ShardedJedis> jedisLocal = new ThreadLocal<ShardedJedis>();

    public RedisCacheManipulater() {
        String addrs = Config.getItem("redis_server"); //redis server,逗号分割
        //InetSocketAddress addr = addrs.iterator().next();
        JedisPoolConfig config = new JedisPoolConfig();
        config.setMaxTotal(1024); // 最大支持1024个并发连接
    ...
    /**
     * 由于jedis不是线程安全的，而且是直接操作Connection的，因此被迫使用连接池以及使用线程栈。
     * 
     * @return Redis客户端
     */
    protected ShardedJedis getJedis() {
        if (pool == null) {
            throw new IllegalStateException("redis client not initilized.");
        }
    ...
    public int delete(final Collection<String> keys) {
        return execute(null, new RedisCallback<Integer>() {
            public Integer doWithJedis(ShardedJedis jedis) {
                ...
                long cnt = 0;
                for (Entry<Jedis, List<String>> e : serverKeyMap.entrySet()) {
    ...
    public Map<String, String> getString(final Collection<String> keys) {
        return execute(null, new RedisCallback<Map<String, String>>() {
            ...
            public Map<String, String> doWithJedis(ShardedJedis jedis) {
                ...
                for (Entry<JedisShardInfo, List<String>> e : shardInfos.entrySet()) {
    ...
    public <T> Map<String, T> get(final Collection<String> keyCollections, final long timeout) {
        ...
        for (Entry<String, byte[]> e : bytesMap.entrySet()) {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public <T> T get(final String key, long timeout) {
    ...
    @Override
    public boolean set(final String key, final int exp, final Object value, long timeout) {
        return execute(key, new RedisCallback<Boolean>() {

            public Boolean doWithJedis(ShardedJedis jedis) {
                String status = "";
                if (exp > 0) {
                    status = jedis.setex(transcoder.encodeString(key), exp, transcoder.encode(value).getFullData());
                } else {
    ...
    public Map<String, byte[]> getBytes(final Collection<String> keys) {
        ...
        return execute(null, new RedisCallback<Map<String, byte[]>>() {
            ...
            public Map<String, byte[]> doWithJedis(ShardedJedis jedis) {
                ...
                //
                for (Entry<JedisShardInfo, List<byte[]>> e : shardInfos.entrySet()) {
